...
def super_resolve_img(generator, input_path, output_path=None, weight_dtype=torch.float32, downsample_threshold=-1, crop_for_4x=True, threshold=1280):
    ...
    h, w, c = img_lr.shape
    if h*w >= threshold * threshold:
        print(f"The image size is too large for {input_path}, we will skip the upscaling process (pixel size is {h*w})")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--upscale_pixel_threshold', type = int, default = 1280, help="The threshold of the pixel size to upscale, if the pixel size is over this threshold, we will skip the upscaling process. Default is 1280 pixels.")
    ...
    if os.path.isdir(store_dir):    # If the input is a directory, we will iterate it
        for filename in tqdm(sorted(os.listdir(input_dir))):
            ...
            super_resolve_img(generator, input_path, output_path, weight_dtype, downsample_threshold, crop_for_4x=True, threshold=args.upscale_pixel_threshold)
        ...
        # In default, we will automatically use crop to match 4x size
        super_resolve_img(generator, input_dir, output_path, weight_dtype, downsample_threshold, crop_for_4x=True, threshold=args.upscale_pixel_threshold)
